...
void AddDot8x4(int n, float* xout, float* x, float* w) {
    ...
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l10_sum), vget_high_f32(y_l10_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l20_sum), vget_high_f32(y_l20_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l30_sum), vget_high_f32(y_l30_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l40_sum), vget_high_f32(y_l40_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l50_sum), vget_high_f32(y_l50_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l60_sum), vget_high_f32(y_l60_sum));
    *y_ptr++ = vpadds_f32(temp);

    temp = vpadd_f32(vget_low_f32(y_l70_sum), vget_high_f32(y_l70_sum));
    *y_ptr++ = vpadds_f32(temp);
}
...
void AddDot8x8(int n, float* xout, float* x, float* w) {
    ...
    for (i = 0; i < n; i += 8) {
        ...
        w_reg0 = vld1q_f32(&W(4, i));
        w_reg4 = vld1q_f32(&W(4, i + 4));
        y_l40_sum = vfmaq_f32(y_l40_sum, w_reg0, x_reg0);
        y_l44_sum = vfmaq_f32(y_l44_sum, w_reg4, x_reg4);

        w_reg0 = vld1q_f32(&W(5, i));
        w_reg4 = vld1q_f32(&W(5, i + 4));
        y_l50_sum = vfmaq_f32(y_l50_sum, w_reg0, x_reg0);
        y_l54_sum = vfmaq_f32(y_l54_sum, w_reg4, x_reg4);

        w_reg0 = vld1q_f32(&W(6, i));
        w_reg4 = vld1q_f32(&W(6, i + 4));
        y_l60_sum = vfmaq_f32(y_l60_sum, w_reg0, x_reg0);
        y_l64_sum = vfmaq_f32(y_l64_sum, w_reg4, x_reg4);

        w_reg0 = vld1q_f32(&W(7, i));
        w_reg4 = vld1q_f32(&W(7, i + 4));
        y_l70_sum = vfmaq_f32(y_l70_sum, w_reg0, x_reg0);
        y_l74_sum = vfmaq_f32(y_l74_sum, w_reg4, x_reg4);
    }
    if (i != n) {
        printf("%s:%d %d != %d\n", __FILE__, __LINE__, i, n);
    }
    ...
    float* y_ptr = xout;
    float32x4_t temp0 = vaddq_f32(y_l00_sum, y_l04_sum);
    float32x2_t temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l10_sum, y_l14_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l20_sum, y_l24_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l30_sum, y_l34_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l40_sum, y_l44_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l50_sum, y_l54_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l60_sum, y_l64_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);

    temp0 = vaddq_f32(y_l70_sum, y_l74_sum);
    temp = vpadd_f32(vget_low_f32(temp0), vget_high_f32(temp0));
    *y_ptr++ = vpadds_f32(temp);
}
